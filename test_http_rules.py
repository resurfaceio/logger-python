...
def test_parses_rules_with_bad_verbs():
    for verb in ["b", "bozo", "*", ".*"]:
        parse_fail('{v}'.format(v=verb))
        parse_fail('!.*! {v}'.format(v=verb))
        parse_fail('/.*/ {v}'.format(v=verb))
        parse_fail('%request_body% {v}'.format(v=verb))
        parse_fail('/^request_header:.*/ {v}'.format(v=verb))


def test_parses_rules_with_invalid_scopes():
    for scope in ["request_body", "*", ".*"]:
        parse_fail('/{s}'.format(s=scope))
        parse_fail('/{s}# 1'.format(s=scope))
        parse_fail('/{s} # 1'.format(s=scope))
        parse_fail('/{s}/'.format(s=scope))
        parse_fail('/{s}/ # 1'.format(s=scope))
        parse_fail(' / {s}'.format(s=scope))
